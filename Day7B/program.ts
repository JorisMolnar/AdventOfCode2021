...
export class Program {
  ...
  private calcResult (input: string): number {
    ...
    const fuels = orderBy(
      range(minPos, maxPos + 1).map(dest => [dest, sum(hPositions.map(start => calcFuelCost(start, dest)))]),
      d => d[1]
    )
...
const calcFuelCost = (start: number, dest: number): number => sum(range(1, Math.abs(start - dest) + 1))
